    ...
    if num < 9999:
        ...
        if i3<25:
            ...
            if i2<25:
                ...
                i1 = letras.index(l1)
                l1 = letras[(i1+1)%26]; l2=l3='A'
        nuevo = f"{l1}{l2}{l3}0000"
    with open(archivo,"a") as f:
        f.write(nuevo+"\n")
    return nuevo

def guardar_registro(folio, entidad):
    with open(REGISTROS_FILE,"a") as f:
        f.write(f"{folio}|{entidad}\n")
...
def formulario_cdmx():
    if request.method=="POST":
        ...
        now   = datetime.now()
        f_exp = now.strftime(f"%d DE {meses_es[now.strftime('%B').lower()]} DEL %Y")
...
def formulario_morelos():
    if request.method=="POST":
        ...
        now   = datetime.now()
        f_l   = now.strftime(f"%d DE {meses_es[now.strftime('%B').lower()]} DEL %Y")
        f_c   = now.strftime("%d/%m/%Y")
        f_ven = (now+timedelta(days=30)).strftime("%d/%m/%Y")
        ...
        pg.insert_text(coords_morelos["fecha"][:2], f_l,
        ...
        if len(doc)>1:
            ...
            doc[1].insert_text((x,y), f_c, fontsize=s, color=col)
        doc.save(out); doc.close()
        guardar_registro(folio, "morelos")
