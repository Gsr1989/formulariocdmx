...
def _guardar(folio, entidad, serie, fecha_exp, fecha_ven, nombre):
    existe = os.path.exists("registros.csv")
    with open("registros.csv", "a", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        if not existe:
            w.writerow(["folio","entidad","serie","fecha_exp","fecha_ven","nombre"])
        w.writerow([folio, entidad, serie, fecha_exp, fecha_ven, nombre])
            ...
            for row in reader:
                if len(row)==6:
                    regs.append({
                        ...
                        "fecha_exp":row[3],"fecha_ven":row[4],"nombre":row[5]
                    })
    return regs
...
def guardar_registros(regs):
    with open("registros.csv","w",newline="",encoding="utf-8") as f:
        ...
        w.writerow(["folio","entidad","serie","fecha_exp","fecha_ven","nombre"])
        for r in regs:
            w.writerow([r["folio"],r["entidad"],r["serie"],
...
def formulario_cdmx():
    ...
    if request.method=="POST":
        ...
        for key in ["marca","serie","linea","motor","anio"]:
            x,y,s,col=coords_cdmx[key]; pg.insert_text((x,y), d[key], fontsize=s, color=col)
        ...
        _guardar(fol,"CDMX",d["serie"],f_exp,f_ven,d["nombre"])
...
def formulario_edomex():
    ...
    if request.method=="POST":
        ...
        doc.save(out); doc.close()
        _guardar(fol,"EDOMEX",d["serie"],f_exp,f_ven,d["nombre"])
...
def formulario_morelos():
    ...
    if request.method=="POST":
        ...
        f_ven=(ahora+timedelta(days=30)).strftime("%d/%m/%Y"); out=os.path.join(OUTPUT_DIR,f"{fol}_morelos.pdf")
        ...
        doc.save(out); doc.close()
        _guardar(fol,"Morelos",d["serie"],f_larga,f_ven,d["nombre"])
...
def formulario_oaxaca():
    ...
    if request.method=="POST":
        ...
        doc.save(out); doc.close()
        _guardar(fol,"Oaxaca",d["serie"],f1,f_ven,d["nombre"])
...
def formulario_gto():
    ...
    if request.method=="POST":
        ...
        doc.save(out); doc.close()
        _guardar(fol,"GTO",d["serie"],f_exp,f_ven,d["nombre"])
...
def renovar(folio):
    ...
    plantillas={
        ...
        "Morelos":"morelos_hoja1_imagen.pdf","Oaxaca":"oaxacachido.pdf",
    ...
    for k in ["marca","serie","linea","motor","anio","color","nombre"]:
        if k in coords_map:
            x,y,s,col=coords_map[k]; pg.insert_text((x,y),viejo.get(k,viejo["nombre"]),fontsize=s,color=col)
    doc.save(out); doc.close()
    _guardar(nuevo,viejo["entidad"],viejo["serie"],f_exp,f_ven,viejo["nombre"])
