...
def guardar(folio, entidad, serie, marca, linea, motor, anio, color, fecha_impresa, fecha_vencimiento, nombre):
# Genera la ruta del archivo local
nombre_pdf = f"{folio}{entidad.lower()}.pdf"
...
def renovar(folio):
    ...
    doc.save(out); doc.close()
    guardar(nuevo, entidad, viejo["serie"], viejo["marca"], viejo["linea"], viejo["motor"], viejo["anio"], viejo["color"], exp_f, ven_f, viejo["nombre"])
    return redirect(url_for(f"abrir_pdf{entidad.lower()}", folio=nuevo))
    # --- DESCARGA PDFs ---
